...
include 'config/connector.php';
  ...
  if (isset($_GET['id_mobil'])) {
    ...
    $query = "SELECT * FROM showroom_mega_table WHERE id_mobil='$id_mobil'";
    $result = mysqli_query($connector, $query);
    // jika data gagal diambil maka akan tampil error berikut
    if(!$result){
      die ("Query Error: ".mysqli_errno($connector).
    ...
    $data = mysqli_fetch_assoc($result);
      // apabila data tidak ada pada database maka akan dijalankan perintah ini
       if (!count($data)) {
          echo "<script>alert('Data tidak ditemukan pada database');window.location='ListCar-Mega.php';</script>";
       }
  } else {
    // apabila tidak ada data GET id pada akan di redirect ke index.php
    echo "<script>alert('Masukkan data id');window.location='ListCar-Mega.php';</script>";
  }         
  ?>
<!doctype html>
<html lang="en">
        ...
        <div class="row">
            ...
            <div class="col">
                <form action="config/edit.php" method="POST" enctype="multipart/form-data">
                    ...
                    <a href="ListCar-Mega.php" class="btn btn-primary">Kembali</a>
                </form>
            </div>
        </div>
    <!-- end form -->
